...
### 边缘网关采集：MQTT、Modbus TCP、OPC UA

边缘网关核心作用是「采集底层设备数据，向上传递至边缘计算节点（如 FactMesh）或云端」。三种主流数据传递方式的核心差异及适用场景如下。

#### 核心差异对比

| 对比维度 | MQTT | Modbus TCP | OPC UA |
|----------|------|------------|--------|
| **协议定位** | 轻量级消息传输（发布/订阅），专注数据分发 | 工业串行通信（主从模式），专注底层设备采集 | 工业通用数据交互（客户端/服务器），专注标准化传输 |
| **传输特性** | 协议头小、断连重连、QoS 分级、一对多分发 | 基于 TCP、协议简单、主从架构、无内置加密 | 平台无关、支持复杂类型、TLS 加密、数据订阅 |
| **适配设备** | 物联网设备、边缘节点、云端服务（如 FactMesh） | 工业底层设备（传感器、PLC、变频器） | 工业控制系统、智能设备、高端 PLC |
| **开发复杂度** | 低，Paho-MQTT 成熟 | 极低，协议简单 | 高，需 Eclipse Milo 等 SDK |
| **核心优势** | 轻量、低耗、远距离、跨节点同步 | 简单、高效、兼容老旧设备 | 标准化、安全、功能强 |
| **核心劣势** | 需网关中转，无内置数据标准化 | 安全性弱、类型有限、不支持一对多 | 资源占用高、开发成本高 |

#### 适用场景（FactMesh 边缘采集）

- **MQTT**：边缘-云端/跨节点数据同步（FactMesh 核心场景）。边缘网关采集后向 FactMesh 发布到 `factmesh/input/device/+`；边缘节点间同步；向下游系统一对多分发。完美对接 `infra/mqtt` 模块，复用 `MqttClientWrapper`。

- **Modbus TCP**：边缘网关→底层工业设备采集（近距离、简单设备）。直接采集 PLC、传感器、变频器等，尤其适配老旧设备（无 OPC UA）。采集后转换为标准化格式，再通过 MQTT 传递给 FactMesh，形成「Modbus 采集→MQTT 传输」链路。

- **OPC UA**：边缘网关→高端智能设备/工业级标准化。采集智能机器人、复杂控制系统；对安全要求高的场景（化工、能源）可启用 TLS；跨厂商设备对接。采集后转换为 FactMesh 事实数据模型，通过 MQTT 或接口传递。

#### 最优组合

三者分工协作：**底层采集**用 Modbus TCP（老旧设备）+ OPC UA（高端设备）；**数据传输**用 MQTT（边缘网关→FactMesh、跨节点同步、下游分发）。优先用 Modbus/OPC UA 完成「边缘网关→设备」采集，用 MQTT 完成「边缘网关→FactMesh」传输。
...
在默认配置、未做专门扩容调优的前提下，系统大致支持的数据量级如下。

### 业务数据（PostgreSQL）

| 维度 | 当前设计 | 预估量级 |
|------|----------|----------|
| 单表数据 | 分页 10–20 条/页，无总条数限制 | 百万～千万级 |
| 连接池 | 每模块默认 10 连接 | - |

单库单表千万级记录可支撑，更大量需分库分表、读写分离、连接池调优。

### 设备遥测（InfluxDB）

| 维度 | 当前设计 | 预估量级 |
|------|----------|----------|
| 写入方式 | WriteApiBlocking 逐点写入 | 百～千 点/秒 |
| 单次查询 | limit 最大 10000 条 | 单次 1 万点 |
| 模拟器 | 设备数上限 1000 | 千台设备 |

适合**百～千台设备**、**秒级**采集频率；更高吞吐需改用批量写入（WriteApi 异步批量）。

### 工业协议采集（OPC UA / Modbus TCP）

| 维度 | 当前设计 | 预估量级 |
|------|----------|----------|
| 连接池 | 各协议池 maxTotal=10 | 约 20 台设备 |

适合**十～几十台**工业设备，更多需增大连接池或优化采集策略。

### 消息传递（MQTT / EMQX）

| 维度 | 当前设计 | 预估量级 |
|------|----------|----------|
| 应用层 | 同步处理，单实例单客户端 | 千级 msg/s |
| Broker | EMQX | 万级 msg/s |

### 综合量级与扩容方向

| 场景 | 当前预估 | 主要瓶颈 | 扩容方向 |
|------|----------|----------|----------|
| 设备数量 | 百～千台 | 遥测写入、工业协议连接池 | InfluxDB 批量写、边缘网关聚合 |
| 遥测吞吐 | 百～千 点/秒 | InfluxDB 单点写 | WriteApi 异步批量 |
| 业务记录 | 百万～千万级 | 连接池、索引 | 读写分离、分库分表 |
| 并发请求 | 数十 QPS/模块 | 连接池、单实例 | 多实例 + 负载均衡 |
| MQTT 消息 | 千～万 msg/s | 应用处理 | 异步消费、消息队列缓冲 |

**总结**：默认配置下可支撑百～千台设备、百～千 点/秒遥测、百万～千万级业务记录；更大规模需按上述方向优化。
