...
| 相关文档 | [DDD 架构总览](DDD_ARCHITECTURE_OVERVIEW.md)、[系统开发计划](SYSTEM_DEVELOPMENT_PLAN.md)、[Knife4j 集成](knife4j-integration.md) |
...
- **统一入口与文档**：前端与外部系统仅对接网关；接口文档通过 Knife4j 在网关层聚合、统一访问。
...
| 网关与聚合 | mom-gateway | 路由、鉴权、限流、Knife4j 文档聚合 |
...
```
                    ...
                    │   路由 / 鉴权 / 限流 / Knife4j 文档聚合 /doc.html  │
                    └─────────────────────────┬───────────────────────┘
                                              │
        ┌─────────────────────────────────────┼─────────────────────────────────────┐
        │                 Nacos（服务注册与发现 + 配置管理）                          │
...
| 文档与规范 | OpenAPI 3、Knife4j、knife4j-gateway-spring-boot-starter | 各服务暴露 `/v3/api-docs`，网关聚合至 `/doc.html` |
...
- **Docker 编排**：使用 `docker-compose-pgsql.yml` 提供 PostgreSQL、MySQL（Nacos 存储）、Nacos；应用可逐步容器化接入。
...
- **mom-common**：工具类、统一响应/异常、OpenAPI/Knife4j 配置、Feign 客户端接口等，无业务表。
...
| 模块 | 职责摘要 | 对外接口形式 |
|------|----------|--------------|
| mom-common | 公共配置、工具、统一 DTO/异常、Knife4j/OpenAPI 配置 | 被其他模块依赖，无 HTTP 端点 |
...
| mom-gateway | 路由、鉴权、限流、Knife4j 聚合 | `/doc.html`、`/api/**` 转发、健康检查等 |
...
- **一域一库**：每个业务域使用独立 PostgreSQL 数据库（ly_factmesh_admin / iot / mes / wms / qms），库结构在 `sql/` 下按域维护。
...
- **文档**：所有接口在各自服务内通过 OpenAPI 3 暴露（Knife4jConfig 在 mom-common），网关通过 Knife4j Gateway 聚合，统一在 `/doc.html` 访问与调试。
...
- **扩展性**：新业务域可新增独立微服务与数据库，仅需在 Nacos 注册并在网关暴露路由；前端与 Knife4j 可随新服务自动扩展。
...
| 注册中心 | Nacos | 2.3.2（见 docker-compose-pgsql.yml） |
| 网关     | Spring Cloud Gateway | 由 BOM 管理 |
| 文档     | Knife4j OpenAPI3 / Gateway | 4.4.0 |
| 数据库   | PostgreSQL | 16.4 |
...
- [Knife4j 集成](knife4j-integration.md)：OpenAPI3 与网关文档聚合配置。
