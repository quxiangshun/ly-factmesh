...
- **Docker 编排**：`tools/docker-compose-nacos.yml`（Nacos+MySQL）、`tools/docker-compose-influxdb.yml`（InfluxDB）、`tools/pgsql`（PostgreSQL 主从）；根目录 `docker-compose.yml` 为完整编排。
...
| 注册中心 | Nacos | 2.3.2（见 tools/docker-compose-nacos.yml） |
