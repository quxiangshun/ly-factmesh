<template>
  <section class="page">
      ...
      <div class="toolbar-actions">
        <div class="title-with-tip">
          ...
          <div v-if="showTip" class="tip-popover" @click.stop>
            <div class="tip-content">跨服务审计记录，谁在何时执行了哪些操作</div>
          </div>
        </div>
        <input v-model="filterService" placeholder="服务名筛选" class="filter-input" />
      <input v-model.number="filterUserId" type="number" placeholder="用户ID" class="filter-input" />
    ...
    <template v-else>
      <div v-if="!pageData?.records?.length" class="empty-state">暂无审计记录</div>
        ...
        <table class="data-table">
          ...
          <tbody>
            <tr v-for="row in pageData?.records" :key="row.id">
              ...
              <td class="content-cell">{{ row.operationContent || '-' }}</td>
              <td>{{ row.operationResult === 1 ? '成功' : '失败' }}</td>
              <td>{{ row.clientIp || '-' }}</td>
